...
def test_stargaze_outdoors_at_night_succeeds():
    ...
    result = execute_handler(StargazeHandler(), _ctx(actor), _cmd(character.id))
    assert result.ok
...
def test_stargaze_lifts_mood():
    actor, _room, character = _world()
    execute_handler(StargazeHandler(), _ctx(actor), _cmd(character.id))
...
def test_charting_a_visible_constellation_logs_it():
    ...
    result = execute_handler(
        StargazeHandler(), _ctx(actor), _cmd(character.id, {"constellation": "the Hare"})
    )
    assert result.ok
...
def test_recharting_a_known_constellation_emits_no_identify_event():
    ...
    execute_handler(handler, _ctx(actor), _cmd(character.id, {"constellation": "the Hare"}))
    result = execute_handler(
        handler, _ctx(actor), _cmd(character.id, {"constellation": "the Hare"})
    )
    assert result.ok
...
def test_telescope_deepens_the_mood_lift():
    ...
    execute_handler(StargazeHandler(), _ctx(plain_actor), _cmd(plain.id))
    execute_handler(StargazeHandler(), _ctx(scope_actor), _cmd(gazer.id))
...
def test_rejects_invalid_character_id():
    ...
    result = execute_handler(StargazeHandler(), _ctx(actor), _cmd("???"))
...
def test_rejects_missing_character():
    actor, _room, _character = _world()
    result = execute_handler(StargazeHandler(), _ctx(actor), _cmd("entity_9999"))
    assert not result.ok
...
def test_rejects_unknown_constellation():
    actor, _room, character = _world()
    result = execute_handler(
        StargazeHandler(), _ctx(actor), _cmd(character.id, {"constellation": "the Toaster"})
    )
    assert not result.ok
